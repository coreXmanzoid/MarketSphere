...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setButtonLoading(btn, isLoading, loadingLabel) {
        ...
        if (isLoading) {
            ...
            btn.innerHTML = '<i class="bi bi-arrow-repeat se-spin"></i> ' + (loadingLabel || 'Saving...');
        } else {
            btn.disabled = false;
   ...
   ========================================================= */
    ...
    /* =========================================================
       ...
       ========================================================= */
    const futureToolButtons = document.querySelectorAll('.se-future-btn.is-disabled');
    ...
    document.querySelectorAll('.se-status-card-btn, .se-edit-btn, .se-future-btn')
        .forEach(bindRipple);

    /* =========================================================
       SCROLL REVEAL (matches home.js / order_detail.js pattern)
       ========================================================= */
    const revealTargets = document.querySelectorAll('.se-card, .se-profile-card');
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const documentCards = document.querySelectorAll(".se-document-card");
    ...
    documentCards.forEach(card => {
        ...
        const previewArea = card.querySelector(".se-document-preview");
...
function attachDocumentEvents() {
    ...
    documentCards.forEach((card) => {
        ...
        card.addEventListener("click", function (e) {
            const uploadBtn = e.target.closest(".btn-hero-primary, .se-upload-document");
            const changeBtn = e.target.closest(".se-change-document");
            const viewBtn = e.target.closest(".se-view-document");
        ...
        if (previewArea) {
            ...
            previewArea.addEventListener("drop", function (e) {
                ...
                if (files && files.length > 0) {
                    const file = files[0];
                    const triggerBtn = card.querySelector(".btn-hero-primary, .se-change-document");
                    handleSelectedFile(file, card, triggerBtn);
                }
            });
        }
...
function updateDocumentCard(card, file, responseData) {
    const previewArea = card.querySelector(".se-document-preview");
    const metaValue = card.querySelector(".se-document-meta-value");
    const statusBadge = card.querySelector(".se-document-status");
    const actions = card.querySelector(".se-document-actions");
    ...
    // Status Badge
    if (statusBadge) {
        statusBadge.className = "se-document-status pending";
    ...
    if (previewArea) {
        if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
            previewArea.innerHTML = `
                <div class="se-document-pdf">
                    <i class="bi bi-file-earmark-pdf-fill se-doc-icon"></i>
                    <span class="se-document-file-name">${file.name}</span>
                </div>
            `;
        } else {
            const reader = new FileReader();
            reader.onload = function (e) {
                previewArea.innerHTML = `<img class="se-document-image" src="${e.target.result}" alt="Document Preview">`;
            };
    ...
    if (actions) {
        actions.innerHTML = `
            <button type="button" class="btn se-edit-btn se-view-document">
                <i class="bi bi-eye"></i> View
            </button>
            <button type="button" class="btn btn-hero-primary se-change-document">
