...
def user_buyers(request):
    ...
    return render(request, "user_management/buyer/buyers.html", {"buyers": total_buyers})
...
def logout_all_devices(request):
    ...
    return JsonResponse({
        "status": "success",
        "message": f"Logged out from {deleted} active device(s)."
    })
...
def update_buyer_profile_view(request, buyer_id):
    ...
    success, message, buyer = update_buyer_profile(
        request=request,
        buyer_id=buyer_id
    )
...
def admin_reset_password(request, user_id):
    ...
    if form.is_valid():
        ...
        return JsonResponse({
            "success": True,
            "message": f"Password reset email sent to {buyer.email}."
        })
